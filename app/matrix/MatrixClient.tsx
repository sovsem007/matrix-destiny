  ...
  if (!dob) {
    return (
      <div className="container py-16 max-w-2xl mx-auto">
        <h1 className="font-heading text-3xl font-bold mb-8 text-center">Матрица судьбы</h1>
        <MatrixForm />
      </div>
    );
  }

  if (loading) return <MatrixPageSkeleton />;
    ...
    <div className="container py-8 md:py-12 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-8 gap-4">
        ...
        <div className="flex gap-2">
          <Button variant="mystic-outline" size="sm" onClick={handleShare} aria-label="Скопировать ссылку">
            <Share2 className="h-4 w-4" />
            {copied ? "Скопировано!" : "Поделиться"}
          </Button>
          <Button variant="outline" size="sm" onClick={handleExportPdf} disabled={pdfLoading} aria-label="Экспорт в PDF">
      ...
      <div id="matrix-pdf-export" ref={resultRef}>
        ...
        <div className="mt-8 grid lg:grid-cols-2 gap-8">
          <div className="space-y-6">
            ...
            >
              <h2 className="font-heading text-xl font-semibold mb-6 text-center">Визуальная схема матрицы</h2>
              <MatrixChart
            ...
            <div>
              <h3 className="font-semibold text-sm text-muted-foreground mb-3 uppercase tracking-wide">Позиции матрицы</h3>
              <MatrixLegend positions={result.positions} highlightedPosition={highlightedPosition} onPositionHover={setHighlightedPosition} />
            </div>
          </div>
          <div className="space-y-4">
            <Tabs defaultValue="interpretations">
              <TabsList className="w-full">
                <TabsTrigger value="interpretations" className="flex-1">Расшифровка</TabsTrigger>
                <TabsTrigger value="keywords" className="flex-1">Ключевые слова</TabsTrigger>
              </TabsList>
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {centerArcana.keywords.map((kw: string) => (
              <span key={kw} className="rounded-full border border-violet-500/30 bg-violet-500/10 px-3 py-1 text-xs text-violet-300">
                {kw}
              </span>
            ))}
          </div>
      ...
      <div className="space-y-3">
        ...
        <p className="text-sm text-muted-foreground leading-relaxed italic">{centerArcana?.advice}</p>
      </div>
    </div>
  );
}
